  ...
  try {
    ...
    if (slugParts[0]) {
      ...
      if (fs.existsSync(sidebarPath)) {
        try {
          ...
          docsTree = generateDocsUsingContents(JSON.parse(sidebarContent))
        } catch (jsonError) {
          console.error(`Error parsing sidebar.json for ${slugParts[0]}:`, jsonError)
        }
      } else if (docRoot) {
    ...
    return (
      <div className="p-6 bg-red-50 text-red-800">
        ...
        <p>There was an error loading this page. Please check the server logs for more details.</p>
      </div>
    )
  }
